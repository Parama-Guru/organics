...
export default async function StoresPage({ searchParams }: PageProps<"/[lang]/stores">) {
  ...
  const stores = await getVerifiedStores(query);
        ...
        <div className="mt-6 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {stores.map((store, index) => (
            ...
            >
              ...
              <dl className="mt-4 space-y-1 text-sm text-bark-600">
                {store.fssaiNumber ? (
                  <div className="flex flex-wrap gap-x-2">
                    <dt className="font-medium text-bark-900">{t.stores.fssai}</dt>
                    <dd className="font-mono">{store.fssaiNumber}</dd>
                  </div>
                ) : null}
                {store.certifier ? (
                  <div className="flex flex-wrap gap-x-2">
                    <dt className="font-medium text-bark-900">{t.stores.certifiedBy}</dt>
