# Organics â€” running checklist

Updated as work lands. `[x]` = done and verified, `[ ]` = not done, `[~]` = done
but blocked on something outside the code.

**Score: 37 done Â· 3 blocked Â· 13 open**

Last updated: 2026-09-01 Â· `main` @ `5fa92b1` Â· CI green

---

## Blocked â€” these stop the site going live
...
- [ ] **`ADMIN_PASSWORD_HASH` is not set in Render.** It is `sync: false`, and
      Render only prompts for those at blueprint *creation* â€” it ignores them on
      every later sync. While empty, `/tj` returns 404 and the admin area cannot
      be reached at all. Generate with `npm run admin:hash`, paste into Render â†’
      Environment.
      The value must match `^scrypt:[0-9a-f]{32}:[0-9a-f]{128}$` â€” exactly 168
...
- [ ] **Confirm the `organics-kv` Key Value instance actually got created.**
      ...
      refuses to boot in prod â€” the same boot loop as before.

## Open â€” decisions needed from you

- [ ] **Login wall: pick 1, 2 or 3.** (1) everything behind login, (2) browse
      free but gate farm detail pages, (3) browse free, sign in only to save â€”
...
## Open â€” work I can do on request
...
- [ ] Rotate the Supabase password, Redis URL and admin passphrase â€” they were
...
- [ ] Shop `photoUrl` column exists but is unused â€” wire it up when shopfront
      photos exist.
...
- [x] Shop detail page at `/stores/[slug]` â€” only VERIFIED resolve, others 404
- [x] Registration at `/stores/register` with FSSAI licence validation
- [x] `POST /api/stores` â€” same-origin, rate limited 3/hour, status not client-settable
...
- [x] Messages stored in the database, not emailed â€” mail is optional config
...
- [x] Careers page â€” states there are no openings rather than inventing roles
...
- [x] Streamed in batches of 500 via cursor pagination â€” no unbounded `findMany`
- [x] Every cell escaped against spreadsheet formula injection
- [x] UTF-8 BOM so Excel on Windows reads the Tamil columns
- [x] 401 without a session, 400 on an unknown type â€” both verified live
...
### SEO
- [x] `sitemap.xml` from the database â€” 50 URLs, Tamil slugs percent-encoded
...
- [x] CI green on every commit â€” lint, config check, typecheck, build, Docker

### Fixes
- [x] `?next=` on the two detail pages, so signing in no longer loses the item
- [x] Shared `isSameOrigin` across all three write routes
- [x] Git history scanned â€” no credentials ever committed
...
- [x] The Redis error message now names the likely cause â€” an unapplied
      blueprint â€” and the escape hatch, instead of just stating the rule.
